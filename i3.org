...
#+BEGIN_SRC conf
  bindsym $mod+s exec quickswitch.py -d "dmenu_run -p 'launch :>' -fn 'Fantasque Sans Mono-12' -sb '#0088CC'"
#+END_SRC
...
#+BEGIN_SRC conf
  ...
  for_window [title="^quake$"] border 1px
...
#+BEGIN_SRC conf
 exec --no-startup-id feh --bg-scale ~/Pictures/wallpaper-2392652.jpg
#+END_SRC
...
** ssh agent
#+BEGIN_SRC sh
exec --no-startup-id eval $(gpg-agent --daemon --enable-ssh-support --write-env-file "${HOME}"/.gpg-agent-info)
#+END_SRC
* gnupg agent
:PROPERTIES:
